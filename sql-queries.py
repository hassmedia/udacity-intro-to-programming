...
'''
This program will create and populate a database, then get specific values using SQL.
'''

# make a Link class with id, submitter id, submission 
# time, number of votes, title, and url
...
# make and populate a database table, only in memory
# for testing
...
# function for selecting values based on submitter id
def query():
...
# function, and another method, for selecting values based on votes
def query_alt():
    c = db.execute("select id from links where votes > 100 order by submitted_time asc")
    results = [t[0] for t in c]
    return results

# print list with values
print "Posts by submitter id 62443: " + str(query())
print "Posts with more than 100 votes: " + str(query_alt())
